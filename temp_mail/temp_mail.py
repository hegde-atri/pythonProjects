...
def checkMails():
    ...
    if length == 0:
        print_statusline(
            "Your mailbox is empty. Hold tight. Mailbox is refreshed automatically every 5 seconds."
        )
    else:
        idList = []
        for i in req:
        ...
        print_statusline(
            f"You received {length} {x}. (Mailbox is refreshed automatically every 5 seconds.)"
        )
...
banner()
userInput1 = input(
    "Do you wish to use to a custom domain name (Y/N): ").capitalize()
